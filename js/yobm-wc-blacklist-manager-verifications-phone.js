jQuery(document).ready(function($) {
	...
	// Function to handle the phone verification process
	function handlePhoneVerification() {
		if ($('.woocommerce-error li .phone-verification-error').length > 0) {
			...
			$('.woocommerce-error li .phone-verification-error').append(`
				<div>
					...
					success: function(response) {
						if (response.success) {
							...
							if (response.data && response.data.message === wc_blacklist_manager_verification_data.resend_limit_reached_message) {
								$('#verification_message').text(response.data.message).show();
								$('#resend_timer').hide();  // Hide the resend timer
								$('#resend_button').hide();  // Hide the resend button
							} else {
